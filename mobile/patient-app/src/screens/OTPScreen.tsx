...
type Props = {
  ...
  route: { params: { phone: string; name: string; role: string; dev_otp?: string } };
};

const RESEND_SECONDS = 60;

export default function OTPScreen({ navigation, route }: Props) {
  const { phone, name, dev_otp } = route.params;
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/auth/verify-otp`, {
        ...
        body: JSON.stringify({ phone, otp, name, role: 'patient' }),
      });
      ...
      await Promise.all([
        ...
        AsyncStorage.setItem('patient_name', name),
        AsyncStorage.setItem('patient_phone', phone),
      ]);
